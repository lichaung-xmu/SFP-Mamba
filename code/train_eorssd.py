...
"""
Official EORSSD training script for the open-source SFP-Mamba release.
...
EORSSD augmentation:
    identity, rot90, rot180, rot270, hflip, hflip+rot180
...
Dataset defaults intentionally retain the author's local paths and can be
...
DEFAULT_RELEASE_CHECKPOINT = REPO_ROOT / "checkpoints" / "sfp_mamba_eorssd.pth"
DEFAULT_OUTPUT = REPO_ROOT / "outputs" / "eorssd_100e"

DEFAULT_TRAIN_IMAGE_ROOT = Path("/home/lch/work/sod/dataset/EORSSD/train-images")
DEFAULT_TRAIN_GT_ROOT = Path("/home/lch/work/sod/dataset/EORSSD/train-labels")
DEFAULT_VAL_IMAGE_ROOT = Path("/home/lch/work/sod/dataset/EORSSD/test-images")
DEFAULT_VAL_GT_ROOT = Path("/home/lch/work/sod/dataset/EORSSD/test-labels")
...
# -----------------------------------------------------------------------------
# EORSSD six-view augmentation
# -----------------------------------------------------------------------------
AUG6_IDS = (0, 1, 2, 3, 4, 6)
AUG6_NAMES = (
    "identity",
    "rot90",
    "rot180",
...
class SixFoldAugmentedDataset(Dataset):
...
def build_sixfold_loader(base_loader, batch_size, seed):
    dataset = SixFoldAugmentedDataset(base_loader.dataset)
...
# -----------------------------------------------------------------------------
# Optional development validation; same protocol as test_eorssd.py
# -----------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Official 100-epoch EORSSD training for SFP-Mamba."
    )
    ...
    print("=" * 120)
    print("SFP-MAMBA | OFFICIAL EORSSD 100-EPOCH TRAINING")
    print("=" * 120)
    ...
    print("Augmentation     : 6x {}".format(AUG6_NAMES))
    ...
    pre_region_loader = build_sixfold_loader(
        base_loader,
        args.pre_region_batchsize,
        args.seed,
    )
    full_loader = build_sixfold_loader(
        base_loader,
        args.full_batchsize,
        args.seed + 100003,
    )
    ...
    print("Effective 6x samples   :", len(pre_region_loader.dataset))
    ...
    (output / "run_config.txt").write_text(
        "\n".join(
            [
                ...
                "resolution=352",
                "augmentation_views={}".format(",".join(AUG6_NAMES)),
                "effective_batch=16",
                "optimizer=AdamW",
