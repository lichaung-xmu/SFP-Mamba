...
class PVMLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask_flat = mask.view(B, 1, n_tokens).expand(B, C, n_tokens).transpose(-1, -2)
            x_norm = x_norm * mask_flat
...
class MSA_mamba_head(nn.Module):
    def __init__(self, mode='dilation', dim=128, num_heads=8, ffn_expansion_factor=4, bias=False,
        ...
        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = PVMLayer(input_dim=dim, output_dim=dim)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)
