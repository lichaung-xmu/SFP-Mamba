...
"""
Official single-view ORSSD evaluation for the open-source SFP-Mamba release.

Default checkpoint:
    checkpoints/sfp_mamba_orssd.pth

Default local ORSSD test data:
    /home/lch/work/sod/dataset/ORSSD/Image-test
    /home/lch/work/sod/dataset/ORSSD/GT-test
...
DEFAULT_CHECKPOINT = REPO_ROOT / "checkpoints" / "sfp_mamba_orssd.pth"
DEFAULT_IMAGE_ROOT = Path("/home/lch/work/sod/dataset/ORSSD/Image-test")
DEFAULT_GT_ROOT = Path("/home/lch/work/sod/dataset/ORSSD/GT-test")
DEFAULT_OUTPUT = REPO_ROOT / "results" / "ORSSD"
...
def evaluate(model, image_root, gt_root, testsize, output_dir, save_predictions, print_every):
    ...
    for index in range(loader.size):
        ...
        if (index + 1) % max(1, int(print_every)) == 0 or (index + 1) == loader.size:
            print("[ORSSD] {}/{}".format(index + 1, loader.size))
...
def main():
    parser = argparse.ArgumentParser(
        description="Official single-view ORSSD evaluation for SFP-Mamba."
    )
    ...
    print("=" * 120)
    print("SFP-MAMBA | OFFICIAL ORSSD SINGLE-VIEW TEST")
    print("=" * 120)
